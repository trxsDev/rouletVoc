...
class GestureClassifier:
    ...
    def classify(landmarks):
        ...
        is_pinched = (thumb_idx_dist < (0.44 * palm_size)) or (thumb_idx_dist < 56.0)

        # -----------------------------------------------------
        # 1. OK Gesture / Thai Jeeb (👌 / 🤏)
        # Thumb & Index touching + at least 2 other fingers open/raised
        # -----------------------------------------------------
        open_fingers_count = int(mid_ext or not mid_crl) + int(rng_ext or not rng_crl) + int(pnk_ext or not pnk_crl)
        if is_pinched and (not mid_crl) and (open_fingers_count >= 2):
            return "OK", thumb_idx_dist, True

        # -----------------------------------------------------
        # 2. PINCH (🤏)
        # Thumb & Index pinched together
        # -----------------------------------------------------
        if is_pinched:
